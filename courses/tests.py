...
class LessonTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.cours = Cours.objects.create(
            name="Основы языка Python", description="Азы программирования на Python"
        )
        self.lesson = Lesson.objects.create(
            name="Что такое программа?", cours=self.cours, owner=self.user
        )
...
class SubscriptionTestCase(APITestCase):
    def setUp(self):
        ...
        self.course = Cours.objects.create(
            name="Курс 7", description="Описание курса 7"
        )
        self.lesson = Lesson.objects.create(
            name="первый урок курса 7", cours=self.course, owner=self.user
        )
